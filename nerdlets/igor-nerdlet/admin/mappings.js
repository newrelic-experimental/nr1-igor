...
import { Icon, AccountStorageMutation, NerdGraphQuery } from 'nr1';
...
export default class Mappings extends React.Component {
    ...
    this.setState(
      {
        attribute: attrib,
        selectedValues:
          (data[selectedLocation].attribute || '') === attrib
            ? data[selectedLocation].values
            : []
      },
  ...
  async fetchAttribs() {
    ...
    const skipAttribs = [
      ...
      'hostID',
      'instanceType',
      'nr.apmApplicationIds',
      'nr.apmApplicationNames',
      'nr.entityType',
      'processorCount',
      'systemMemoryBytes',
      'vendor'
    ...
    return data ? (
      <div className="cols bound">
                  ...
                  >
                    <div>
                      <div className="location">
                        {loc}
                        {'values' in data[loc] &&
                          data[loc].values.length > 0 && (
                            <span className="count">
                              {data[loc].values.length}
                            </span>
                          )}
                      </div>
        ...
        {data && !selectedLocation && (
          <div
            className="col"
            style={{ justifySelf: 'center', padding: '1em' }}
          >
            <Icon type={Icon.TYPE.INTERFACE__CHEVRON__CHEVRON_LEFT} />
            <span style={{ font: '1.5em Inconsolata, monospace' }}>
              select location
            </span>
          </div>
        )}
        {selectedLocation && (
          <div className="col" style={{ padding: '1em' }}>
            <div className="field">
              <label htmlFor="host-types" className="label">
                Show
              </label>
              <div className="select">
                <span
                  style={{ position: 'absolute', top: '45%', right: '1.5em' }}
                >
                  <Icon
                    type={
              ...
              <ul className="hosts">
                ...
                {values
                  .filter(
                    v =>
                      searchText === '' ||
                      v.toLowerCase().indexOf(searchText.toLowerCase()) !== -1
                  )
                  .map((v, i) => (
                    <li key={i}>
                      <input
      ...
      <div style={{ font: '1.5em Inconsolata, monospace' }}>
        No location(s). Add location(s) first.
      </div>
    );
  }
}
